...
public:
    ...
    Simulator() : pcMuxPc(""), pcTemp(""), ir(""), rd(nullopt), rs1(""), rs2(""), func3(""), func7(""),
...
int main() {
    Simulator sim;
    ifstream inputFile("output.mc");
    sim.memory["0x10000000"] = "4";
    if (!inputFile) {
        cerr << "Error: Could not open file output.mc" << endl;
        return 1;
    }
    ...
    while (getline(inputFile, line)) {
        ...
        // Normalize PC to 8-character hex format (0x00000004)
        stringstream formattedPC;
        formattedPC << "0x" << uppercase << setfill('0') << setw(8) << hex << stoul(pc, nullptr, 16);

        sim.textSegment[formattedPC.str()] = ir;
    }
    inputFile.close();

    // Set initial PC (normalized format)
    sim.pcMuxPc = "0x00000000";

    

    // Execution loop
    while (sim.textSegment.find(sim.pcMuxPc) != sim.textSegment.end()) {
        sim.fetch();
        sim.decode();
        sim.execute();
        sim.memoryAccess();
